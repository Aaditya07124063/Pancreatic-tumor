"""
Scratch CNN - Pancreatic CT Binary Classification
Custom CNN trained from scratch | 5-seed evaluation | 80/10/10 stratified split
"""
...
BATCH_SIZE = 32
LR = 1e-3
OUTPUT_DIR = "./cnn_scratch_outputs"
...
def train_one_seed(seed, all_paths, all_labels):
    ...
    model = ScratchCNN(num_classes=len(CLASSES)).to(DEVICE)
    ...
    torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, f"cnn_scratch_seed{seed}.pt"))
    return {
...
def plot_confusion_matrix(cm, seed):
    ...
    ax.set(xticks=[0, 1], yticks=[0, 1], xticklabels=CLASSES, yticklabels=CLASSES,
           ...
           title=f"Scratch CNN Confusion Matrix (seed={seed})")
...
def plot_training_curves(history, seed):
    ...
    ax2.legend()
    plt.suptitle(f"Scratch CNN Training Curves — seed {seed}")
    plt.tight_layout()
...
def main():
    print("=" * 65)
    print("  Scratch CNN — Pancreatic CT Classification  |  5-Seed Eval")
    print("=" * 65)
    ...
    pd.DataFrame(rows).to_csv(os.path.join(OUTPUT_DIR, "cnn_scratch_results.csv"), index=False)
