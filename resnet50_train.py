"""
ResNet50 - Pancreatic CT Binary Classification
...
def plot_confusion_matrix(cm, seed, class_names=CLASSES):
    ...
    ax.set(xticks=list(ticks), yticks=list(ticks),
           ...
           title=f"ResNet50 Confusion Matrix (seed={seed})")
...
def plot_training_curves(history, seed):
    ...
    ax2.legend()
    plt.suptitle(f"ResNet50 Training Curves — seed {seed}")
    plt.tight_layout()
...
def main():
    ...
    pd.DataFrame(rows).to_csv(
        os.path.join(OUTPUT_DIR, "resnet50_results.csv"), index=False)
