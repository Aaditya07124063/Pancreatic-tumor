"""
MobileViT - Pancreatic CT Binary Classification
Uses HuggingFace transformers (MobileViTForImageClassification)
...
def train_one_seed(seed, all_paths, all_labels):
    ...
    torch.save(model.state_dict(),
               os.path.join(OUTPUT_DIR, f"mobilevit_seed{seed}.pt"))
...
def plot_confusion_matrix(cm, seed, class_names=CLASSES):
    ...
    ax.set(xticks=list(ticks), yticks=list(ticks),
           ...
           title=f"MobileViT Confusion Matrix (seed={seed})")
...
def plot_training_curves(history, seed):
    ...
    ax2.legend()
    plt.suptitle(f"MobileViT Training Curves — seed {seed}")
    plt.tight_layout()
...
def main():
    print("=" * 65)
    print("  MobileViT — Pancreatic CT Classification  |  5-Seed Eval")
    print("=" * 65)
    ...
    pd.DataFrame(rows).to_csv(
        os.path.join(OUTPUT_DIR, "mobilevit_results.csv"), index=False)
